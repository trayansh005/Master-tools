    ...
    <div className="min-h-screen font-sans bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="bg-brand-green px-4 text-white py-2 flex justify-between items-center">
        <div className="text-center" style={{ width: '88%' }}>
          {user ? (
            <span className="text-sm">Welcome, {user.name}! We deliver to over 150 countries, including the UK and EU nations, in 2-4 days! Click here to learn more about delivery to your area.</span>
          ) : (
        ...
        <div className="space-x-4">
          {user ? (
            <>
              {user.name === "admin" ? (
                <Link href="/admin/" className="text-sm">Admin Dashboard</Link>
              ) : (
            ...
            <>
              ...
              <Link href="/auth/register" className="bg-[var(--accent-green)] hover:bg-[var(--accent-lightyellow)] text-[var(--background)] hover:text-[var(--accent-dark)] px-3 py-1 rounded text-sm">Sign Up</Link>
            </>
          )}
        </div>
      </div>
